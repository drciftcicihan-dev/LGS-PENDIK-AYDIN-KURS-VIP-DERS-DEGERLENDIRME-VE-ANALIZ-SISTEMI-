...
def get_db() -> sqlite3.Connection:
    ...
    conn.execute("PRAGMA synchronous=NORMAL;")  # WAL modu performans ve disk güvenliği
    conn.execute("PRAGMA foreign_keys=ON;")
    return conn


def create_database_backup(reason="daily"):
    """SQLite online backup; application verilerini engellemeden yedek alır."""
...
def replace_all_data(payload: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    ...
    with get_db() as conn:
        ...
        # Değişikliklerin anında veritabanı dosyasına işlenmesini garanti eder
        conn.commit()
